...
function lottery_odds($type,$ball,$h){
	global $mysqli;
	$sql	=	"select * from c_odds_22 where type='".$ball."' limit 1";
...
function six_odds($class,$type,$number){
	...
	global $mysqli;
	$sql	=	"select * from c_odds_22 where type='ball_".$class."'";
	$query	=	$mysqli->query($sql);
	$t		=	$query->fetch_array();
	if($class==14){
		$odds = 0;
...
function lottery_qishu($type) {
	...
	global $mysqli;
	if($type==0){
		$sql	=	"select qishu from c_opentime_22 where kaipan<='".date("H:i:s",$lottery_time)."' and fengpan>='".date("H:i:s",$lottery_time)."' limit 1";
		//var_dump($sql);
		$query	=	$mysqli->query($sql);
		//var_dump($mysqli);
