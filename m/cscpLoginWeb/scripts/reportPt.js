function dateGetTotalReportDayPT(){
	$("#Yesterday").click(function() {
		$("#BeginDate").val($("#YBeginDate").val());
		$("#EndDate").val($("#YEndDate").val());
		getTotailRepotTotailDayPT();
	});
...
function getTotailRepotTotailDayPT(){
	$("#returnBg").html("");
...
function addPTSlotsType(){
	var o = {
	};
	var jsonuserinfo = $.toJSON(o);
	$.ajax({
		type : "post",
		url : $("#path").val() + "/app/addPTSlotsType?" + Math.random()*10000,
		data : jsonuserinfo,
		contentType : 'application/json',
		dataType : "json",
		async:true, 
		...
		success : function(data) {
			if(data){
				if(data.success){
					for ( var i = 0; i < data.slotsPTTypes.length; i++) {
						$("#LiveType").append( "<option value='" + data.slotsPTTypes[i] + "'>" + l_SlotsPtGameType["pt" + data.slotsPTTypes[i]] + "</option>");
					}
				}
			}
		},
...
function dateGetLiveBetDetailPT(){
	$("#Yesterday").click(function() {
		$("#BeginDate").val($("#YBeginDate").val());
		$("#EndDate").val($("#YEndDate").val());
		getLiveBetDetailPT('1');
	});
	$("#Today").click(function() {
		$("#BeginDate").val($("#TBeginDate").val());
		$("#EndDate").val($("#TEndDate").val());
...
function getLiveBetDetailPT(pageNumber){
	$("#pageNext").unbind();
	$("#pagePrev").unbind();
	var fromDateObj = $("#BeginDate");
	var toDateObj = $("#EndDate");
...
//		hall:$("#Hall").val(),
	...
	var jsonuserinfo = $.toJSON(o);
	 ...
	 $.ajax({
				...
				type : "post",
				url : $("#path").val() + "/app/getLiveBetDetailPT?" + Math.random()*10000,
				data : jsonuserinfo,
				contentType : 'application/json',
				dataType : "json",
				async : true,
							...
							if (data.resultList.length > 0) {
								...
								for ( var i = 0; i < data.resultList.length; i++) {
									if(platform == "PTSLOTS") {
										trHtml = "<tr>";
										trHtml += "<td>"+ (i + 1)+ "</td>"
											+ "<td>" + l_SlotsPtGameType["pt" + data.resultList[i].gameNameCode] + "</td>" 
											+ "<td>" + data.resultList[i].gameCode + "</td>" 
											...
											+ "<td class='F_bold f_right'>" + formatNumberWinLoss(data.resultList[i].progressiveBet,2) + "</td>" 
											+ "<td class='F_bold f_right'>" + formatNumberWinLoss(data.resultList[i].progressiveWin,2) + "</td>" 
											+ "<td class='F_bold f_right'>" + formatNumberWinLoss(data.resultList[i].currentBet,2) + "</td>" 
								...
								for ( var j = forBegin; j <= forEnd; j++) {
									if (j != pageNum) {
										pageNumHtml += "<a onclick=\"getLiveBetDetailPT("+ j+ ");\" style='color:#000;cursor: pointer;'>"+ j+ "</a>&nbsp;";
									} else {
								...
								$("#pageMember").hide();
								var noRecordString = "";
								noRecordString = "<td colspan='10'>您当前没有讯息</td>";
								trHtml = "<tr id='noRecord' align='center'>"
									+ noRecordString
								+ "</tr>";
