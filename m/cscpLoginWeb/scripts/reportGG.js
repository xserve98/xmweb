function dateGetTotalReportDayGG(){
	$("#Yesterday").click(function() {
		$("#BeginDate").val($("#YBeginDate").val());
		$("#EndDate").val($("#YEndDate").val());
		getTotailRepotTotailDayGG();
	});
...
function getTotailRepotTotailDayGG(){
	$("#returnBg").html("");
...
function addGGSlotsType(){
	var o = {
	};
	var jsonuserinfo = $.toJSON(o);
	$.ajax({
		type : "post",
		url : $("#path").val() + "/app/addGGSlotsType?" + Math.random()*10000,
		data : jsonuserinfo,
		contentType : 'application/json',
		dataType : "json",
		async:true, 
		...
		success : function(data) {
			if(data){
				if(data.success){
					for ( var i = 0; i < data.slotsGGTypes.length; i++) {
						$("#LiveType").append( "<option value='" + data.slotsGGTypes[i] + "'>" + l_SlotsGgGameType[data.slotsGGTypes[i]] + "</option>");
					}
				}
			}
		},
...
function dateGetLiveBetDetailGG(){
	$("#Yesterday").click(function() {
		$("#BeginDate").val($("#YBeginDate").val());
		$("#EndDate").val($("#YEndDate").val());
		getLiveBetDetailGG('1');
	});
	$("#Today").click(function() {
		$("#BeginDate").val($("#TBeginDate").val());
		$("#EndDate").val($("#TEndDate").val());
...
function getLiveBetDetailGG(pageNumber){
	$("#pageNext").unbind();
	$("#pagePrev").unbind();
	var fromDateObj = $("#BeginDate");
	var toDateObj = $("#EndDate");
...
//		hall:$("#Hall").val(),
	...
	var jsonuserinfo = $.toJSON(o);
	 $.ajax({
				type : "post",
				url : $("#path").val() + "/app/getLiveBetDetailGG?" + Math.random()*10000,
				data : jsonuserinfo,
				contentType : 'application/json',
				dataType : "json",
				async : true,
//				timeout : 200000,
				beforeSend : function() {
					...
					var joinWhatPage = "";
					if(platform == "GGSLOTS") {
						joinWhatPage = "slotsGGWinLossDay";
					}
							...
							if (data.resultList.length > 0) {
								...
								for ( var i = 0; i < data.resultList.length; i++) {
									...
									if(platform == "GGSLOTS") {
										trHtml = "<tr>";
										trHtml += "<td>"+ (i + 1)+ "</td>"
											+ "<td>" + l_SlotsGgGameType[data.resultList[i].gameId] + "</td>" 
											+ "<td>" + data.resultList[i].userName + "</td>" 
											+ "<td>" + formartTime(data.resultList[i].betTime,1) + "</td>"
											+ "<td>" + data.resultList[i].linkId + "</td>" 
								...
								for ( var j = forBegin; j <= forEnd; j++) {
									if (j != pageNum) {
										pageNumHtml += "<a onclick=\"getLiveBetDetailGG("+ j+ ");\" style='color:#000;cursor: pointer;'>"+ j+ "</a>&nbsp;";
									} else {
