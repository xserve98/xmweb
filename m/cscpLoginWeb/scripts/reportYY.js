function dateGetTotalReportDayYY(){
	$("#Yesterday").click(function() {
		$("#BeginDate").val($("#YBeginDate").val());
		$("#EndDate").val($("#YEndDate").val());
		getTotailRepotTotailDayYY();
	});
...
function getTotailRepotTotailDayYY(){
	$("#returnBg").html("");
...
function addYYSlotsType(){
	var o = {
	};
	var jsonuserinfo = $.toJSON(o);
	$.ajax({
		type : "post",
		url : $("#path").val() + "/app/addYYSlotsType?" + Math.random()*10000,
		data : jsonuserinfo,
		contentType : 'application/json',
		dataType : "json",
		async:true, 
		...
		success : function(data) {
			if(data){
				if(data.success){
					for ( var i = 0; i < data.slotsYYTypes.length; i++) {
						$("#LiveType").append( "<option value='" + data.slotsYYTypes[i] + "'>" + l_SlotsYyGameType[data.slotsYYTypes[i]] + "</option>");
					}
				}
			}
		},
...
function dateGetLiveBetDetailYY(){
	$("#Yesterday").click(function() {
		$("#BeginDate").val($("#YBeginDate").val());
		$("#EndDate").val($("#YEndDate").val());
		getLiveBetDetailYY('1');
	});
	$("#Today").click(function() {
		$("#BeginDate").val($("#TBeginDate").val());
		$("#EndDate").val($("#TEndDate").val());
...
function getLiveBetDetailYY(pageNumber){
	$("#pageNext").unbind();
	$("#pagePrev").unbind();
	var fromDateObj = $("#BeginDate");
	var toDateObj = $("#EndDate");
...
//		hall:$("#Hall").val(),
	...
	var jsonuserinfo = $.toJSON(o);
	 $.ajax({
				type : "post",
				url : $("#path").val() + "/app/getLiveBetDetailYY?" + Math.random()*10000,
				data : jsonuserinfo,
				contentType : 'application/json',
				dataType : "json",
				async : true,
//				timeout : 200000,
				beforeSend : function() {
					...
					var joinWhatPage = "";
					if(platform == "YYSLOTS") {
						joinWhatPage = "slotsYYWinLossDay";
					}
							...
							if (data.resultList.length > 0) {
								...
								for ( var i = 0; i < data.resultList.length; i++) {
									...
									if(platform == "YYSLOTS") {
										trHtml = "<tr>";
										trHtml += "<td>"+ (i + 1)+ "</td>"
											+ "<td>" + l_SlotsYyGameType[data.resultList[i].gameKey] + "</td>" 
											+ "<td>" + data.resultList[i].gameSetKey + "</td>" 
											...
											+ "<td class='F_bold font_r f_right'>" + "已派彩" + "</td>" 
											+ "</tr>";
								...
								for ( var j = forBegin; j <= forEnd; j++) {
									if (j != pageNum) {
										pageNumHtml += "<a onclick=\"getLiveBetDetailYY("+ j+ ");\" style='color:#000;cursor: pointer;'>"+ j+ "</a>&nbsp;";
									} else {
