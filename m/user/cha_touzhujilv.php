...
$sql		=	"select bid from k_bet where `status`>0 and uid=$uid and match_coverdate>='$stime' and match_coverdate<'$etime' and (ball_sort!='重庆时时彩' AND ball_sort!='广东快乐十分' AND ball_sort!='北京PK10' AND ball_sort!='福彩3D' AND ball_sort!='体彩排列三' AND ball_sort!='上海时时乐') ".$fsql." order by bid desc";
}else
{
	$sql		=	"select bid from k_bet where `status`>0 and uid=$uid and match_coverdate>='$stime' and match_coverdate<'$etime' and (ball_sort='重庆时时彩' or ball_sort='广东快乐十分' or ball_sort='北京PK10' or ball_sort='福彩3D' or ball_sort='体彩排列三' or ball_sort='上海时时乐') ".$fsql." order by bid desc";
}
