...
for($i=1;$i<=180;$i++){
	echo $i."===";
	$sqls="select * from c_opentime_26 where qishu='$i'";
	echo $sqls;
	...
	if($rs['qishu']){
		...
		$strSqls="UPDATE `c_opentime_26` SET `kaipan`='$actiontime',`fengpan`='$fengpan',`kaijiang`='$kaijiang' WHERE (`qishu`='".$rs['qishu']."')";
		echo $strSqls."<br>";
...
if($pankou=='A'){
	$sql		= "select * from c_odds_26 order by id asc";	
	//$sql		= "select * from c_odds_26_".strtolower($pankou)."order by id asc";	
	////echo $sql;
}else{
...
if($qs){
	...
	$sql		= "select * from c_opentime_26 where qishu=132";
	$query		= $mysqli->query($sql);
	$qs		= $query->fetch_array();
	if($qs){
		$qishu		= date("Ymd",$lottery_time).BuLings($qs['qishu']);
