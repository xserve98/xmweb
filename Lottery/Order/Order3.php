...
$did=date("YmdHis");
//广东快乐十分
if ($_REQUEST['type'] == 3 || $_REQUEST['type'] == 11 || $_REQUEST['type'] == 19 || $_REQUEST['type'] == 28) {
    //获取彩种
    ...
    //获取期数
    if($_REQUEST['type'] == 28) {
        $qishu = lottery_qishu_28($_REQUEST['type']);
    ...
    if($qishu == -1 || $qh != $qishu) {
        echo json_encode(array('code' => 2, 'info' => $qishu.'已经封盘，禁止下注！'));
        exit;
	}
    unset($datas['qi_num']);
    //获取清空后的POST键名
