...
function loadinfo(type) {
    ...
    g_type = type;
	$.post("class/odds_27.php", function(data) {
		if(data.opentime >= 0) {
...
function auto(ball) {
    ...
    $.post("class/auto_27.php?"+ Date.parse(new Date()),{ball: ball}, function(data) {
	...
	var str =new_hm2+'<div class="result_stat clearfix"><div class="statitm">'+hz+'</div><div class="statitm">'+hzdx+'</div><div class="statitm">'+hzds+'</div><div class="statitm">'+b1lh+'</div></div>'
			...
			var kjqh ='<div><div>澳洲幸运5</div><div>'+new_qh+'期开奖</div></div>';
		 ...
		 $("#result_balls", parent.document).addClass("T_SSC L_CQSSC");
            ...
            win_parent.find("#gm_name").html("澳洲幸运5");
