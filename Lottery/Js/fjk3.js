...
function loadinfo(type) {
    ...
    g_type = type;
	$.post("class/odds_31.php", function(data) {
		if(data.opentime > 0) {
...
function auto(ball) {
	...
	$.post("class/auto_31.php", {ball: ball}, function(data) {
		...
		if(qihao== openqishu){
		  ...
		  window.parent.document.getElementById('hk_mp3').innerHTML= "<embed src='/date/kaijiang.mp3' width='0' height='0'></embed>";
		   ...
		   if(hz>22){
			...
			var kjqh ='<div>福建快三</div><div>'+new_qh+'期开奖</div>';
			var win_parent = $(window.parent.document);
		 ...
		 $("#result_balls", parent.document).addClass("T_K3 L_K3");
            win_parent.find("#gm_name").html("福建快三");
