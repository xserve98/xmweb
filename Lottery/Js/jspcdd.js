...
function loadinfo() {
    ...
    $.post("class/odds_26.php", function(data) {
...
function auto() {
    ...
    $.post("class/auto_26.php", function(data) {
        $("#user_sy").html(data.shuying);
      
	  var name =kj_qishu.html().replace(/<\/?.+?>/g,"").substring(0,5);
	/// console.log(name);
if(name === '极速赛车(PK10)'  && qihao == data.kj_list[0]['qishu'] ){
	qihao=1;
	}
  ...
  var hzstr ='<span><b class="bg b_'+hz+'">'+hz+'</b></span>';
            for(var i = 0; i < data.kj_list.length; i++) {
                ...
                for(var j in data.kj_list[i]) {
                    ...
                    if(j == 'qishu') {
                        ls_kj += '<td><i>' + data.kj_list[i][j].substring(4,13) + '</i></td>';
                    } else {
                       if(j == 'ball_1') {
			...
			var kjqh ='<div>极速PC蛋蛋</div><div>'+new_qh+'期开奖</div></div>';
			var win_parent = $(window.parent.document);
