...
function loadinfo(type) {
    ...
    g_type = type;
	$.post("class/odds_21.php", function(data) {
		if(data.opentime >= 0) {
...
function auto(ball) {
    ...
    $.post("class/auto_21.php?"+ Date.parse(new Date()),{ball: ball}, function(data) {
	...
	var str =new_hm2+'<div class="result_stat clearfix ml4"><div class="statitm">'+hz+'</div><div class="statitm">'+hzdx+'</div><div class="statitm">'+hzds+'</div><div class="statitm">'+b1lh+'</div></div>'
		  
					
			var kjqh ='<div>极速时时彩</div><div>'+new_qh+'期开奖</div></div>';
		 ...
		 $("#result_balls", parent.document).addClass("T_SSC L_CQSSC");
			
        
			
		   
            win_parent.find("#gm_name").html("极速时时彩");
...
function history(ball) {
    $.post("class/auto_21.php", {ball: ball}, function(data) {
        if(data.kj_list.length > 0) {
            ...
            var win_parent = $(window.parent.document);
            win_parent.find("#gm_name").html("幸运分分彩");
            win_parent.find("#kj_list").html(ls_kj);
            win_parent.find("#user_order").html('').hide();
            win_parent.find("#info").show();
        }
