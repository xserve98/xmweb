...
function loadinfo() {
    ...
    $.post("class/odds_23.php", function(data) {
        if(data.opentime > 0) {
...
function auto() {
    ...
    $.post("class/auto_23.php", function(data) {
        $("#user_sy").html(data.shuying);
       var qihao = kj_qishu.html().replace(/[^0-9]/ig,"").toString().substring(2,12);
      /// console.log('qihao'+qihao);console.log(data.kj_list[0]['qishu']);console.log('openqishu'+openqishu);
  ...
  var hzstr ='<span><b class="bg b_'+hz+'">'+hz+'</b></span>';
			...
			var kjqh ='<div>新加坡28</div><div>'+new_qh+'期开奖</div></div>';
			var win_parent = $(window.parent.document);
		 ...
		 $("#result_balls", parent.document).addClass("T_PCEGG L_PCEGG");
            ...
            var win_parent = $(window.parent.document);
            win_parent.find("#gm_name").html("新加坡28");
