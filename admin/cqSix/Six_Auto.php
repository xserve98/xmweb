...
if($_GET["action"]=="add" && $id==0){ 
	...
	$sql		=	"insert into c_auto_22(qishu,opentime,endtime,datetime,ball_1,ball_2,ball_3,ball_4,ball_5,ball_6,ball_7) values (".$qishu.",'".$opentime."','".$endtime."','".$datetime."',".$ball_1.",".$ball_2.",".$ball_3.",".$ball_4.",".$ball_5.",".$ball_6.",".$ball_7.")";
	$mysqli->query($sql);
	...
	$ball_6		=	$_POST["ball_6"];
	$ball_7		=	$_POST["ball_7"];
	$sql		=	"update c_auto_22 set qishu=".$qishu.",datetime='".$datetime."',ball_1=".$ball_1.",ball_2=".$ball_2.",ball_3=".$ball_3.",ball_4=".$ball_4.",ball_5=".$ball_5.",ball_6=".$ball_6.",ball_7=".$ball_7." where id=".$id."";
	$mysqli->query($sql);
...
<div class="body">
<?php include_once("../lottery/Menu_Auto.php"); ?>
...
<?php
$sql		=	"select id from c_auto_22 order by qishu desc";
$query		=	$mysqli->query($sql);
...
if($id){
	$id	=	rtrim($id,',');
	$sql	=	"select * from c_auto_22 where id in($id) order by qishu desc";
	$query	=	$mysqli->query($sql);
	$time	=	time();
	while($rows = $query->fetch_array()){
		...
		if($rows['ok']==1){
			...
			$ok = '<a href="Auto/Six.php?qi='.$rows['qishu'].'"   onclick="return js();"><font color="#0000FF">未结算</font></a>';
		}
?>
      <tr>
