...
if($save=='ok'){
	...
	elseif($type==9){
		$sql	=	"update c_odds_22 set h1=".$_REQUEST['Num_1'].",h2=".$_REQUEST['Num_2'].",h3=".$_REQUEST['Num_3'].",h4=".$_REQUEST['Num_4']." where type='ball_".$type."'";
		$mysqli->query($sql);
	...
	elseif($type==15){
		...
		$mysqli->query($sql);
		$type==17 ? $h=2 : $h=1;
		$sql="update c_odds_22 set h$h=".$_REQUEST['fs']." where type='ball_18'";
		$mysqli->query($sql);
	}
	/*
	//写入缓存
	$sql="SELECT * FROM `c_odds_22` order by id";
	...
	if(!write_file("../../cqSix/Odds/6hc.html",$json_string)){ //写入缓存失败
...
//读取当前期数盘口赔率与投注总额
function loadinfo(){
	$.get("get_odds_22.php?t=<?=time()?>", {type : <?=$type?>}, function(data)
		{
...
<div class="body">
<ul id="tab" class="tab">
                <li><input type="button" value="极速六合赔率" hidefocus class="current"/></li>
			</ul>
