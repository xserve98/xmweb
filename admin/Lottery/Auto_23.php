...
?>
  ...
  <tr>
    ...
    <table width="100%" border="0" cellpadding="5" cellspacing="1" class="font12" style="margin-top:5px;" bgcolor="#798EB9">   
        ...
        <td align="center"><strong>普通</strong></td>
        <td align="center"><strong>龙虎斗</strong></td>
...
if($id){
	...
	while($rows = $query->fetch_array()){
		...
		if($rows['ball_1']>$rows['ball_3']){
	$lh='龙';
	}
	
else if($rows['ball_1']==$rows['ball_3']){
		if($rs['ball_1']<5){
		$lh='龙';	
		}else{
		$lh='虎';		
		...
		if($rows['ok']==1){
			$ok = '<a href="../cj/lottery/js_23.php?qi='.$rows['qishu'].'&t=1"><font color="#FF0000">已结算</font></a>';
		}else{
			$ok = '<a href="../cj/lottery/js_23.php?qi='.$rows['qishu'].'&t=1" onclick="return js();" ><font color="#0000FF">未结算</font></a>';
		}
?>
