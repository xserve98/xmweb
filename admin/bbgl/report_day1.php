...
<div id="pageMain">
	...
	$out	=	"#ffffff";
	 ...
	 $where="where `m_make_time`>='$cn_q_btime' and `m_make_time`<='$cn_q_etime' and guest=0";
	 $where2="where `addtime`>='$cn_q_btime' and `addtime`<='$cn_q_etime'  and guest=0";
...
while($rows = $query->fetch_array()){
 $userrow[]=	$rows;  
	...
	$sqla= "select tzname,sum(m_value) as fd from k_money3    ".$where." group by tzname";
