...
$sql		=	"insert into k_user(username,password,qk_pwd,ask,answer,sex,birthday,mobile,email,reg_ip,login_ip,login_time,pay_name,top_uid,agents,is_stop,gid,dltype,zhancheng,lognum,reg_address,zfb,ag_zr_is,is_daili) values ('$username','$password','$qkpwd','$ask','$answer','$sex','$birthday','$mobile','$email','".$_SERVER['REMOTE_ADDR']."','".$_SERVER['REMOTE_ADDR']."','$logintime','$pay_name',$top_uid,'$jsr',$is_stop,1,'$dltype','$zhancheng',1,'$address','$zfb',0,1)";
//echo $sql;
//exit;
...
try{
	...
	$sql	=	"insert into `k_send_back` (`k_type`,`uid`,`k_typename`,`k_wftype`,`k_a_limit`,`k_b_limit`,`k_c_limit`,`k_d_limit`,`k_e_limit`) VALUES ('".$rows['k_type']."',$id,'".$rows['k_typename']."','".$rows['k_wftype']."','".$rows['k_a_limit']."','".$rows['k_b_limit']."','".$rows['k_c_limit']."','".$rows['k_d_limit']."','".$rows['k_e_limit']."')";
	$mysqli->query($sql);
