...
if($_REQUEST['ac']=='re'){
	...
	$sql		= "select * from c_auto_18 where qishu=".$qi." order by id desc limit 1";
}else{
	$sql		= "select * from c_auto_18 where ok=0";
}
...
while ($rs = $query->fetch_assoc()) {
    ...
    //根据期数读取未结算的注单
    $sql1 = "select * from c_bet where type='新加坡快乐8' and js=0 and qishu=" . $qi . " order by addtime asc";
    ...
    $msql = "update c_auto_18 set ok=1 where qishu=" . $qi . "";
...
//防漏单处理
$sql_l = "select qishu from c_bet where type='新加坡快乐8'  and js=0 and qishu=".$qi."";
...
if($sum_l > 0) {
    while($rows = $query_l->fetch_assoc()) {
        $msql = "update c_auto_18 set ok=0 where qishu=" . $rows['qishu'] . "";
...
if ($_GET['t']==1)    {
echo "<script>window.location.href='../../Lottery/auto_1.php';</script>";
}
if($_REQUEST['ac']=='re'){
	echo "OK";
