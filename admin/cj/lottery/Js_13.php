...
while($rows = $query1->fetch_array()){
	...
	$money2 =	$rows['money'];
	///$assets=$assets+$allmoney-
	$about   = '加拿大28开奖派奖返回本金'.$rows['money'];
	$order = $rows['did'];
    $assets  =	$row1['money'];
...
//防漏单处理
$sql_l = "select qishu from c_bet where type='加拿大28'  and js=0 and qishu=".$qi."";
$query_l = $mysqli->query($sql_l);
$sum_l = $mysqli->affected_rows;
if($sum_l > 0) {
    while($rows = $query_l->fetch_assoc()) {
        $msql = "update c_auto_13 set ok=0 where qishu=".$qi."";
