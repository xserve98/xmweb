...
if($_REQUEST['ac']=='re'){
	...
	$sql		= "select * from c_auto_21 where qishu=".$qi." order by id desc limit 1";
}else{
	$sql		= "select * from c_auto_21 where ok=0";
}


$sql1		= "select qishu from c_bet2 where type='极速分分彩' and js=0  and guest=0 order by addtime asc";

$query1		= $mysqli->query($sql1);
$sum		= $mysqli->affected_rows;
$time1=get_total_millisecond();
if( $web_site['ffcyl']==1&&$sum>0){
...
$k=0;
$time1 =get_total_millisecond();
$sql2		= "select mingxi_1,mingxi_2,sum(money) as summoney,sum(win) as sumwin  from c_bet2 where type='极速分分彩' and js=0 and qishu=".$qi." and guest=0 group by mingxi_1,mingxi_2 order by addtime asc";
$query2	= $mysqli->query($sql2);
$sum	= $mysqli->affected_rows;
	
$sqlq="select * from c_auto_21 where  qishu=".$qi." and yl=0 "	;
...
while(true){
   ...
   $time2 =get_total_millisecond();
	...
	$sql2 =	"update c_auto_21 set ball_1=".$arr['ball_1'].",ball_2=".$arr['ball_2'].",ball_3=".$arr['ball_3'].",ball_4=".$arr['ball_4'].",ball_5=".$arr['ball_5'].",yl=1 where qishu=".$qi."";
...
$sql1		= "select * from c_bet where type='极速分分彩' and js=0 and qishu=".$qi." order by addtime asc";
$query1		= $mysqli->query($sql1);
$sum		= $mysqli->affected_rows;
...
//根据期数读取未结算的单
$sql2		= "select * from c_bet where type='极速分分彩' and js=0 and qishu=".$qi." order by addtime asc";
$query2	= $mysqli->query($sql2);
$sum		= $mysqli->affected_rows;
while($rows = $query2->fetch_array()){
...
$msql="update c_auto_21 set ok=1 where qishu=".$qi."";
$mysqli->query($msql) or die ("期数修改失败!!!");
//防漏单处理
$sql_l = "select qishu from c_bet where type='极速分分彩'  and js=0 and qishu=".$qi."";

$query_l = $mysqli->query($sql_l);
$sum_l = $mysqli->affected_rows;
if($sum_l > 0) {
    while($rows = $query_l->fetch_assoc()) {
        $msql = "update c_auto_21 set ok=0 where qishu=".$qi."";
...
if ($_GET['t']==1)    {
	echo "<script>window.location.href='../../Lottery/auto_20.php?type=21';</script>";
}
if($_REQUEST['ac']=='re'){
	echo "OK";
