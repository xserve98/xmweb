...
$hm[]		= $rs['ball_8'];
//根据期数读取未结算的注单
$sql1		= "select * from c_bet where type='重庆幸运农场' and js=0 and qishu=".$qi." order by addtime asc";
...
//防漏单处理
$sql_l = "select qishu from c_bet where type='重庆幸运农场' and js=0 and qishu=".$qi."";
$query_l = $mysqli->query($sql_l);
$sum_l = $mysqli->affected_rows;
if($sum_l > 0) {
    while($rows = $query_l->fetch_assoc()) {
        $msql = "update c_auto_11 set ok=0 where qishu=".$qi."";
...
if ($_GET['t']==1)    {
echo "<script>window.location.href='../../Lottery/auto_3.php?type=11';</script>";
}
if($_REQUEST['ac']=='re'){
	echo "OK";
