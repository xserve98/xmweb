...
if($_REQUEST['ac']=='re'){
	...
	$sql		= "select * from c_auto_27 where qishu=".$qi." order by id desc limit 1";
}else{
	$sql		= "select * from c_auto_27 where ok=0";
}
...
$hm[]		= $rs['ball_4'];
$hm[]		= $rs['ball_5'];
$sql1		= "select * from c_bet where type='澳洲幸运5' and js=0 and qishu=".$qi." order by addtime asc";
...
//根据期数读取未结算的单
$sql2		= "select * from c_bet where type='澳洲幸运5' and js=0 and qishu=".$qi." order by addtime asc";
...
$msql="update c_auto_27 set ok=1 where qishu=".$qi."";
$mysqli->query($msql) or die ("期数修改失败!!!");

//防漏单处理
$sql_l = "select qishu from c_bet where type='澳洲幸运5'  and js=0 and qishu=".$qi."";
$query_l = $mysqli->query($sql_l);
$sum_l = $mysqli->affected_rows;
if($sum_l > 0) {
    while($rows = $query_l->fetch_assoc()) {
        $msql = "update c_auto_27 set ok=0 where qishu=".$qi."";
