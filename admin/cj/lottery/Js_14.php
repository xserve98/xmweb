...
//根据期数读取未结算的单
$sql2		= "select * from c_bet where type='新疆时时彩' and js=0 and qishu=".$qi." order by addtime asc";
...
//防漏单处理
$sql_l = "select qishu from c_bet where type='新疆时时彩'  and js=0 and qishu=".$qi."";
$query_l = $mysqli->query($sql_l);
$sum_l = $mysqli->affected_rows;
if($sum_l > 0) {
    while($rows = $query_l->fetch_assoc()) {
        $msql = "update c_auto_14 set ok=0 where qishu=".$qi."";
...
if ($_GET['t']==1)    {
	echo "<script>window.location.href='../../Lottery/auto_2.php?type=14';</script>";
}
if($_REQUEST['ac']=='re'){
	echo "OK";
