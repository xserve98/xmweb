...
//根据期数读取未结算的单
$sql2		= "select * from c_bet where type='重庆时时彩' and js=0 and qishu=".$qi." order by addtime asc";
...
//防漏单处理
$sql_l = "select qishu from c_bet where type='重庆时时彩'  and js=0 and qishu=".$qi."";
$query_l = $mysqli->query($sql_l);
$sum_l = $mysqli->affected_rows;
if($sum_l > 0) {
    while($rows = $query_l->fetch_assoc()) {
        $msql = "update c_auto_2 set ok=0 where qishu=".$qi."";
