 ...
 session_start();
	...
	//if($_GET['opstate'] == "0"){
		...
		$assets=$rs2['money'];
				...
				$show1 = $mysqli->query($sql1);
				file_put_contents("./show1log.txt", json_encode($sql1)."\r\n",FILE_APPEND);
				file_put_contents("./show1log.txt", json_encode($show1)."\r\n",FILE_APPEND);
				////////////chu处理订单状态///
				$sql2= "update k_user set money = money+". $ovalue ." where uid=" . $_GET['attach'] ;		
				$show2 = $mysqli->query($sql2);
				file_put_contents("./show2log.txt", json_encode($sql2)."\r\n",FILE_APPEND);
				file_put_contents("./show2log.txt", json_encode($show2)."\r\n",FILE_APPEND);
				////////处理会员余额////////
				...
				$show3 = $mysqli->query($sql3);
				file_put_contents("./show3log.txt", json_encode($sql3)."\r\n",FILE_APPEND);
				file_put_contents("./show3log.txt", json_encode($show3)."\r\n",FILE_APPEND);
