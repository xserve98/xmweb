...
<form name="form1" method="GET" action="data_h_money.php" >
			<div class="row">
			...
			if($_GET["end"]){
			 $where .=" and `adddate`>='$begintime' and `adddate`<='$endtime'";
			}
	  	$sql	=	"select id from huikuan where uid=$uid  ".$where."  order by id desc";
	///	echo $sql;
            ...
            while($rows = $query->fetch_array()) {
                ?>
						...
						<tr class="borderbtmgrey">
							<td class="algleft"><?=date("Y-m-d H:i:s",strtotime($rows["adddate"]))?></td>
                            <?php if($rows["cztype"]==1){?>
                            
							<td>在线充值</td>
							<td><?=sprintf("%.2f",abs($rows["money"]))?></td>
							<td ><?=$rows["orderid"]?></td>
							<td><?php if($rows["bank"]=="992"){
								echo "支付宝";
                        ...
                        if($rows["status"] == 1) {
                            ...
                            echo '<span style="color:blue">未支付</span>';
                        }
                        ?> </td>
							...
							<td><?=sprintf("%.2f",abs($rows["money"]))?></td>
							<td ><?=$rows["orderid"]?></td>
							<td><?php if($rows["bank"]=="alipay"){
								echo "支付宝转账";
								}else if($rows["bank"]=="wxpay"){
									echo "微信转账";
									}else {
										echo "银行转账";
...
<?=$page->get_htmlPage("data_h_money.php?")?>
