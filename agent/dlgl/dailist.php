...
$sql		=	"select * from k_user where uid='$suid' limit 1";
$query		=	$mysqli->query($sql);
$rstop			=	$query->fetch_array();
	  
		  $arr=array('大股东','股东','总代理','代理');
for($i=$rstop['dltype']-1;$i<4;$i++){
?>
           <td width="80" align="center"><a href="dailist.php?dltype=<?=$i+1?>" style="color:#0000FF;"><?=$arr[$i]?></a></td> 
...
?>
        ...
        <td width="80" align="center"><a href="dailist.php?top_uid2=<?=$suid?>" style="color:#0000FF;">直属下级</a></td>
...
<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#CCCCCC">
  <tr>
    <td height="24" nowrap bgcolor="#FFFFFF"><table width="100%" border="1" bgcolor="#FFFFFF" bordercolor="#96B697" cellspacing="0" cellpadding="0" style="border-collapse: collapse; color: #225d9c;" id=editProduct   idth="100%" >       <tr style="background-color: #EFE" class="t-title"  align="center">
        ...
        <td width="5%" ><strong>账户余额</strong></td>
...
$sql		=	"select * from k_user where is_daili=1 and dltype='$dltype' and  concat(',',parents,',') like '%,".intval($suid).",%' ";
if(isset($_GET["stop"])){
	
	$sql	.=	" and `is_stop`=".$_GET["stop"] ;
}
 if(isset($_GET["top_uid2"])){
	 $sql		=	"select * from k_user where is_daili=1 and  concat(',',parents,',') like '%,".intval($suid).",%'  ";
...
?>
            ...
            <td><a href="../hygl/user_show.php?id=<?=$rows["uid"]?>"><?=$rows["uid"]?></a></td>
              ...
              <td><?=$rows['money']?></td>
              <td><a href="../cwgl/agentset_money.php?uid=<?=$rows["uid"]?>&type=add">余额修改</a></td>
...
?>
    ...
    <tr><td style="color:red">代理余额合计：<?=$sum_money?></td></tr>
