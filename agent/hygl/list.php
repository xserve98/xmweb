...
session_start();
$suid=$_SESSION["suid"];
$sql	=	"select distinct u.uid,ul.is_login as ul_type from k_user u left join k_group g on u.gid=g.id left join k_user_login ul on u.uid=ul.uid where u.is_delete=0  and is_daili=0  and concat(',',u.parents,',') like '%,".intval($suid).",%' ";
...
if(isset($_GET["dluid"])) $sql.=" and concat(',',u.parents,',') like '%,".intval($_GET["dluid"]).",%' ";
$order	=	"";
...
<table width="100%">
      <tr>
       ...
       <td width="104" align="center"><a href="?top_uid=<?=$suid?>">直属会员</a></td>
       <td width="104" align="center"><a href="user_add.php">新增会员</a></td>
...
<table width="100%" border="1" bgcolor="#FFFFFF" bordercolor="#96B697" cellspacing="0" cellpadding="0" style="border-collapse: collapse; color: #225d9c;" id=editProduct   idth="100%" >       <tr style="background-color: #EFE" class="t-title"  align="center">
		...
		<td width="8%" height="20" ><strong>用户名/真人账号</strong></td>
        <td width="10%" ><strong>真实姓名/注册时间</strong></td>
        <td width="15%" ><strong>财务/<a href="list.php?order=money">余额</a>/支付宝</strong></td>
         <td width="6%" ><strong>设置回水</strong></td>
          <td width="6%" ><strong>盘口</strong></td>
		...
		if($rows["is_stop"]==1){ //停用和删除会员都是灰色
      	...
      	?>
	        <tr align="center" onMouseOver="this.style.backgroundColor='<?=$over?>'" onMouseOut="this.style.backgroundColor='<?=$out?>'" style="background-color:<?=$color?>">
              ...
              <td><a href="../cwgl/hccw.php?username=<?=$rows["username"]?>">查看财务</a>&nbsp;|&nbsp;<br /><font color=blue><b><?=$rows["money"]?></b></font><br /></td>
			  <td><a href="send_back.php?uid=<?=$rows["uid"]?>&username=<?=$rows["username"]?>">回水设置</a><br /></td>
	      	<td><a href="user_show.php?id=<?=$rows["uid"]?>"><?=$rows["pankou"]?>盘</a></td>
