	...
	if($_POST["type"]=="add"){ //加钱
	   ...
	   $sql=	"insert into k_money(uid,m_value,status,m_order,about,assets,balance,type) values('".$top_uid."',$pay_value,1,'$orderid','$about',".$agmoney.",'$assets',3)";
		$mysqli->query($sql);	
			...
			message('加钱成功','set_money.php?uid='.$uid."&type=add");
...
$sql	=	"select username,parents,top_uid from k_user where uid=".$_GET["uid"]." limit 1";
...
<body>
 <table align="center"  width="100%" >
  ...
  <tr>
   ...
   <tr>
    <td width="16%" height="45" align="right">代理账户余额：</td>
    <td width="84%" align="left"><font color="Red"><?=$agmoney?></font></td>
  </tr>
  <tr>
    <td height="49" align="right">加款：</td>
    <td align="left">
    <input name="type" type="radio" value="add" <?=$_GET['type']=='add' ? 'checked="checked"' : ''?>/><span style="color:#009900">加款*只能给直属会员加款</span>&nbsp;&nbsp;&nbsp;&nbsp; 
  </td>
  </tr>
  ...
  <tr>
    <td align="center">&nbsp;</td>
    <td align="left"><input name="submit" type="submit" style="width:100px;"  onclick="return confirm('确定提交?');" value="确定"/>                      <input style="width:100px;"  type="button" value="返回" onclick="javascript :history.back(-1);"></td>
  </tr>
</table>  
    </form>
</td>
